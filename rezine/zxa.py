...
"""
    ...
    Rezine eXtended Atom.  This module sounds like it has a ridiculous
...
from rezine.models import Post, User
from rezine.utils.text import build_tag_uri
from rezine.utils.dates import format_iso8601
from rezine.utils.xml import escape, XML_NS
from rezine.utils.zeml import dump_parser_data


ATOM_NS = 'http://www.w3.org/2005/Atom'
ZINE_NS = 'http://rezine.pocoo.org/'
...
<!--
    ...
    This is a Rezine eXtended Atom file.  It is a superset of the Atom
...
-->
<feed xmlns="%(atom_ns)s" xmlns:rezine="%(rezine_ns)s" xml:lang="%(language)s">\
<title>%(title)s</title>\
<subtitle>%(subtitle)s</subtitle>\
<id>%(id)s</id>\
<generator uri="http://rezine.pocoo.org/"\
 version="%(version)s">RezineXA Export</generator>\
<link href="%(blog_url)s"/>\
<updated>%(updated)s</updated>'''
XML_EPILOG = '</feed>'


NAMESPACES = {None: ATOM_NS, 'rezine': ZINE_NS}
    ...
    def _dump_post(self, post):
        ...
        for category in post.categories:
            ...
            if category.description:
                self.rezine.description(category.description, parent=element)
