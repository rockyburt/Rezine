...
"""
    ...
    To identify a plugin metadata is used. Rezine requires a file
    named `metadata.txt` to load some information about the plugin.

    Rezine currently supports the following metadata information:
...
from rezine.utils.mail import split_email, is_valid_email, check
from rezine.utils.exceptions import UserException, summarize_exception
from rezine.i18n import RezineTranslations as Translations, lazy_gettext, _
from rezine.environment import BUILTIN_PLUGIN_FOLDER
...
def get_object_name(obj):
    ...
    if inspect.isclass(obj) or inspect.isfunction(obj):
        ...
        cls = obj.__class__
    if cls.__module__.startswith('rezine.plugins.'):
        prefix = cls.__module__.split('.', 2)[-1]
    elif cls.__module__.startswith('rezine.'):
...
class FilesystemPlugin(Plugin):
    ...
    @cached_property
    def is_bundled(self):
        """This property is True if the plugin is bundled with Rezine."""
    ...
    def module(self):
        ...
        try:
            # we directly import from the rezine module space
            return __import__('rezine.plugins.%s' % self.name, None, None,
                              ['setup'])
        except:
...
sys.modules['rezine.plugins'] = _plugins = ModuleType('rezine.plugins')
