...
try:
    ...
    from rezine._core import MissingDependency
...
class CommandLineInterface(object):
    ...
    def script(self, argv):
        ...
        parser.add_option('-r', '--repository-id', default='Rezine', dest='repo_id',
                          help='the repository id (default: Rezine)')
        parser.add_option(
            '-f', '--filename',
            help="file name (without spaces and/or version number)")
        options, args = parser.parse_args(argv)
        description = ' '.join(args)
        manage = ManageDatabase(self.get_rezine_instance())
...
class ManageDatabase(object):
    ...
    def cmd_upgrade(self, repo_id=None, version=None, **opts):
        ...
        from rezine.models import SchemaVersion
        if repo_id is None:
            # get all repos, sort those with id "Rezine*" first
            available_svs = sorted(
                SchemaVersion.query.order_by(SchemaVersion.repository_id.asc()),
                key=lambda sv: not sv.repository_id.startswith('Rezine'))
        else:
