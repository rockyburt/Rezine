...
from rezine.i18n import _, lazy_gettext
from rezine.importers import Importer, Blog, Tag, Category, Author, Post, Comment
from rezine.forms import FeedImportForm
from rezine.utils import log
from rezine.utils.admin import flash
from rezine.utils.dates import parse_iso8601
from rezine.utils.xml import Namespace, to_text
...
from rezine.zxa import ZINE_NS, ATOM_NS, XML_NS, ZINE_TAG_URI, ZINE_CATEGORY_URI


rezine = Namespace(ZINE_NS)
...
class AtomParser(Parser):
    ...
    def parse_post(self, entry):
        ...
        post = Post(
            ...
            # content.  This is the default Rezine behavior and makes sense
            # for Rezine.  However nearly every blog works differently and
...
class ZEAExtension(Extension):
    """Handles Rezine Atom extensions.  This extension can handle the extra
    namespace used for ZEA feeds as generated by the Rezine export.  Because
    in a feed with Rezine extensions the rules are pretty strict we don't
    ...
    def __init__(self, app, parser, root):
        ...
        self._tags = {}
        self._categories = {}
        self._dependencies = root.find(rezine.dependencies)

        self._lookup_user = etree.XPath('./rezine:user[@dependency=$id]',
                                        namespaces={'rezine': ZINE_NS})

    def _parse_config(self, element):
        result = {}
        if element is not None:
            for element in element.findall(rezine.item):
    ...
    def _get_author(self, dependency):
        ...
        if author is None:
            element = self._lookup_user(self._dependencies,
                                        id=str(dependency))[0]
            author = Author(
                element.findtext(rezine.username),
                element.findtext(rezine.email),
                element.findtext(rezine.real_name),
                element.findtext(rezine.description),
                element.findtext(rezine.www),
                element.findtext(rezine.pw_hash),
                _to_bool(element.findtext(rezine.is_author)),
                _pickle(element.findtext(rezine.extra))
            )
            for privilege in element.findall(rezine.privilege):
    ...
    def handle_root(self, blog):
        blog.configuration.update(self._parse_config(
            blog.element.find(rezine.configuration)))

    def postprocess_post(self, post):
        if post.parser_data is not None:
            return
        post.parser_data = _parser_data(post.element.findtext(rezine.parser_data))
        content = _get_text_content(post.element.findall(atom.content),
                                    fallback=False)
        if content is not None:
            post.body = content
        content_type = post.element.findtext(rezine.content_type)
        if content_type is not None:
            post.content_type = content_type

    def lookup_author(self, author, entry, username, email, uri):
        dependency = author.attrib.get(rezine.dependency)
    ...
    def parse_comments(self, post):
        ...
        for element in post.element.findall(rezine.comment):
            author = element.find(rezine.author)
            dependency = author.attrib.get('dependency')
            if dependency is not None:
                author = self._get_author(dependency)
                email = www = None
            else:
                email = author.findtext(rezine.email)
                www = author.findtext(rezine.uri)
                author = author.findtext(rezine.name)

            body = _get_html_content(element.findall(rezine.content))
            comment = Comment(author, body, email, www, None,
                              parse_iso8601(element.findtext(rezine.published)),
                              element.findtext(rezine.submitter_ip), 'html',
                              _to_bool(element.findtext(rezine.is_pingback)),
                              int(element.findtext(rezine.status)),
                              element.findtext(rezine.blocked_msg),
                              _parser_data(element.findtext(rezine.parser_data)))
            comments[int(element.attrib['id'])] = comment
            parent = element.findtext(rezine.parent)
            if parent:
