...
"""
    ...
    This module implements the central application object :class:`Rezine`
...
from rezine import _core
from rezine.environment import SHARED_DATA, BUILTIN_TEMPLATE_PATH, \
     BUILTIN_PLUGIN_FOLDER
from rezine.database import db, cleanup_session
from rezine.cache import get_cache
from rezine.utils import ClosingIterator, local, local_manager, dump_json, \
     htmlhelpers
from rezine.utils.datastructures import ReadOnlyMultiMapping
from rezine.utils.exceptions import UserException
...
class Theme(object):
    ...
    def author_info(self):
        ...
        from rezine.utils.mail import split_email
    ...
    def html_contributors_info(self):
        from rezine.utils.mail import check, is_valid_email
        result = []
...
class Rezine(object):
    ...
    Even though the :class:`Rezine` class is a regular Python class, you can't
    create instances by using the regular constructor.  The only documented way
    to create this class is the :func:`rezine._core.setup` function or by using
    one of the dispatchers created by :func:`rezine._core.get_wsgi_app`.
    """
    ...
    def __init__(self, instance_folder):
        # this check ensures that only setup() can create Rezine instances
        if get_application() is not self:
            raise TypeError('cannot create %r instances. use the '
                            'rezine._core.setup() factory function.' %
        ...
        import rezine
        from rezine.urls import make_urls
        from rezine.views import all_views, content_type_handlers, \
             admin_content_type_handlers, absolute_url_handlers
        from rezine.services import all_services
        from rezine.parsers import all_parsers
        ...
        from rezine.notifications import NotificationManager, \
        ...
        env.globals.update(
            ...
            rezine={
                'version':      rezine.__version__,
                'copyright':    _(u'Copyright %(years)s by the Rezine Team')
                                % {'years': '2008-2009'}
            }
        )
        ...
        # register Rezine's upgrade repository
        from rezine.upgrades import REPOSITORY_PATH
        self.register_upgrade_repository('Rezine', REPOSITORY_PATH)
    ...
    def register_upgrade_repository(self, repo_id, repo_path):
        ...
        """
        from rezine.models import SchemaVersion
        from rezine.pluginsystem import Plugin
        ...
        try:
            ...
            db.session.rollback()
            from rezine.database import metadata, schema_versions
    ...
    def check_if_upgrade_required(self):
        ...
        rezine._core.InstanceUpgradeRequired.
        """
        from rezine.models import SchemaVersion
        from rezine.upgrades.customisation import Repository
        ...
        if to_upgrade:
            # set Rezine in maintenance mode
            cfg = self.cfg.edit()
    ...
    def add_shared_exports(self, name, path):
        ...
        plugin.  Per default Rezine will shared the data on it's own but
    ...
    def add_config_var(self, key, field):
        ...
        that are not prefixed are reserved for Rezine' internal usage.
        The `field` is an instance of a field class from rezine.utils.forms
    ...
    def add_absolute_url(self, handler):
        ...
        This is for example used to implement the pages support in Rezine.
        """
    ...
    def add_parser(self, name, class_):
        ...
        :class:`rezine.parsers.BaseParser`.
        """
    ...
    def get_page_metadata(self):
        ...
        result = [
            htmlhelpers.meta(name='generator', content='Rezine'),
            ...
            htmlhelpers.script(url_for('core/shared', filename='js/Rezine.js')),
        ...
        request = get_request()
        javascript = [
            'Rezine.ROOT_URL = %s' % dump_json(base_url),
            'Rezine.BLOG_URL = %s' % dump_json(base_url +
                                             self.cfg['blog_url_prefix'])
        ]
        if request is None or request.user.is_manager:
            javascript.append('Rezine.ADMIN_URL = %s' %
                              dump_json(base_url +
    ...
    def send_error_notification(self, request, error):
        from rezine.notifications import send_notification_template, ZINE_ERROR
    ...
    def dispatch_request(self, request):
        ...
        try:
            ...
            # Some database screwup?! Don't let Rezine stay dispatching 500's
            db.session.rollback()
        ...
        if self.cfg['database_debug'] and \
           ...
           isinstance(response.response, (list, tuple)):
            from rezine.utils.debug import inject_query_info
    ...
    def dispatch_wsgi(self, environ, start_response):
        ...
        # XXX: Remove 'admin_prefix' references for Rezine 0.3
    ...
    def perform_subrequest(self, path, query=None, method='GET', data=None,
        ...
        """Perform an internal subrequest against Rezine.  This method spawns a
        separate thread and lets an internal WSGI client answer the request.
        The return value is then converted into a rezine response object and
        returned.
...
from rezine import i18n
from rezine.utils import log
from rezine.utils.net import NetException
from rezine.utils.http import make_external_url
