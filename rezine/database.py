...
"""
    ...
    Basically you will never use the `rezine.database` module except if you
    are a core developer, but always the high level :mod:`~rezine.database.db`
    module which you can import from the :mod:`rezine.api` module.
...
from rezine.utils import local_manager
...
class ConnectionDebugProxy(ConnectionProxy):
    ...
    def cursor_execute(self, execute, cursor, statement, parameters,
        ...
        try:
            ...
            from rezine.application import get_request
            from rezine.utils.debug import find_calling_context
            request = get_request()
...
sys.modules['rezine.database.db'] = db = ModuleType('db')
key = value = mod = None
