...
from rezine.privileges import assert_privilege, \
...
from rezine.i18n import _, ngettext
from rezine.application import get_request, url_for, emit_event, \
     render_response
...
from rezine.importers import list_import_queue, load_import_dump, \
     delete_import_dump
from rezine.pluginsystem import install_package, InstallationError, \
     get_object_name
from rezine.pingback import pingback, PingbackError
from rezine.forms import ChangePasswordForm, PluginForm, \
...
def render_admin_response(template_name, _active_menu_item=None, **values):
    ...
    if request.app.cfg['maintenance_mode'] and \
        ...
        flash(_(u'Rezine is in maintenance mode. Don\'t forget to '
...
def basic_options(request):
    ...
    if request.method == 'POST' and form.validate(request.form):
        ...
        # to wait for Rezine to reload first which is why we do the
...
@require_admin_privilege(BLOG_ADMIN)
def plugins(request):
    """Load and unload plugins and reload Rezine if required."""
    form = PluginForm()
...
def configuration(request):
    ...
    the values are not further checked it could easily be that Rezine is
...
def information(request):
    """Shows some details about this Rezine installation.  It's useful for
    debugging and checking configurations.  If severe errors in a Rezine
    ...
    from rezine import environment, __version__ as rezine_version
    ...
    if export:
        response.headers['Content-Disposition'] = 'attachment; ' \
            'filename="rezine-environment.html"'
...
def help(req, page=''):
    """Show help page."""
    from rezine.docs import load_page, get_resource
