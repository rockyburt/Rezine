...
from rezine.i18n import lazy_gettext, _, list_timezones, list_languages
from rezine.utils import log
from rezine.utils.forms import TextField, IntegerField, BooleanField, \
    ChoiceField, CommaSeparated
from rezine.utils.validators import ValidationError, is_valid_url_prefix, \
    is_valid_url_format, is_netaddr, is_valid_email
from rezine.application import InternalError
...
DEFAULT_VARS = {
    ...
    'blog_title':               TextField(default=l_(u'My Rezine Blog')),
    'blog_tagline':             TextField(default=l_(u'just another Rezine blog')),
    ...
    'passthrough_errors':       BooleanField(default=_dev_mode,
        help_text=l_(u'If this is set to true, errors in Rezine '
    ...
    # for future Rezine versions that code must be altered first.
...
class Configuration(object):
    ...
    def get_detail_list(self):
        ...
        def sort_func(item):
            ...
            move the implicit "rezine" to the index.
            """
            if item[0] == 'rezine':
                return 1
    ...
    def get_public_list(self, hide_insecure=False):
        ...
        """
        from rezine.application import emit_event
        from rezine.database import secure_database_uri
        result = []
        for key, field in self.config_vars.iteritems():
            value = self[key]
            if hide_insecure:
...
class ConfigTransaction(object):
    ...
    def commit(self):
        ...
        try:
            ...
            rezine_section = sections.pop('rezine')
            sections = [('rezine', rezine_section)] + sorted(sections.items())
