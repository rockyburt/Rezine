...
"""
    ...
    i18n tools for Rezine.  This module provides various helpers for
    ...
    Rezine loads afterwards are stored in pickles rather than MO files.
    ...
    The extracting of strings (either for Rezine core or plugins) is done
...
import rezine
from rezine.environment import LOCALE_PATH, LOCALE_DOMAIN, \
     USE_GETTEXT_LOOKUP
...
def load_core_translations(locale):
    ...
    This only loads the translations for Rezine itself and not the
    ...
    """
    return RezineTranslations.load(LOCALE_PATH, locale, LOCALE_DOMAIN,
                                 USE_GETTEXT_LOOKUP)
...
class RezineTranslations(TranslationsBase, _CustomAttrsTranslations):
    ...
    def load(cls, path, locale=None, domain='messages',
        ...
        if catalog:
            return RezineTranslations(fileobj=open(catalog, 'rb'), locale=locale)
        return RezineNullTranslations(locale=locale)
...
class RezineNullTranslations(NullTranslations, _CustomAttrsTranslations):
...
def get_translations():
    ...
    try:
        return rezine.application.get_application().translations
...
def per_language_string(en, **languages):
    ...
    """
    def lookup():
        app = rezine.application.get_application()
...
def parse_datetime(string, rebase=True):
    ...
    cfg = rezine.application.get_application().cfg
...
def get_timezone(name=None):
    ...
    """
    if name is None:
        name = rezine.application.get_application().cfg['timezone']
...
def serve_javascript(request):
    ...
    if code is None:
        ...
        code = 'Rezine.addTranslations(%s)' % dump_json(dict(
            messages=dict((k.id, k.string) for k in t.client_keys),
            plural_expr=t.plural_expr,
            locale=str(t.locale)
        ))
        _js_translations[request.app] = code
    response = rezine.application.Response(code, mimetype='application/javascript')
    response.add_etag()
