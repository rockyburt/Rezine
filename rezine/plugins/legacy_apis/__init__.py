...
from rezine.api import get_request, url_for, db
from rezine.utils.xml import XMLRPC, Fault
from rezine.utils.forms import BooleanField
from rezine.models import User, Post, Category, Tag, STATUS_PUBLISHED, \
     STATUS_DRAFT
from rezine.privileges import CREATE_ENTRIES, CREATE_PAGES, BLOG_ADMIN, \
     MANAGE_CATEGORIES
...
def login(username, password):
    ...
    # inside Rezine work on the request of this user.
    request = get_request()
    request.user = user
    return request
...
def select_parser(app, struct, default='html'):
    ...
    message and code.
    """
    parser = struct.get('rezine_parser')
    if parser is None:
        return default
